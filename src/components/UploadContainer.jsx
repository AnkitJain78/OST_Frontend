...
const UploadContainer = () => {
  ...
  const sendEmail = () => {
    ...
    setSendMail(true);
    axios
      .post(`${process.env.REACT_APP_API_URL}/api/v1/email/`, {
        ...mailData,
  ...
  const uploadFile = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_API_URL}/api/v1/upload/`, formData, options)
      .then((res) => {
  ...
  return (
    <div className="max-w-lg mx-auto bg-white shadow-md w-full rounded-[10px] p-[32px] border">
      ...
      {sendMail && (
        <a href={`${process.env.REACT_APP_API_URL}${downloadUrl}`}>
          <button className={twMerge(
            "bg-primary px-[14px] py-[9px] text-center text-white font-medium mt-5 rounded-[4px] cursor-pointer hover:scale-[1.03] hover:bg-primary/80 duration-200 ease-in-out w-full"
          )}>Download</button>
        </a>
      )}
